...
class DashboardIntegration:
    ...
    async def dashboard_guild_settings(self, guild: discord.Guild, **kwargs) -> typing.Dict[str, typing.Any]:
        ...
        # Custom template to show names and IDs beside fields
        custom_form_html = (
            '{{ form.alert_channel.label }} {{ form.alert_channel() }} '
            '(<b>{{ alert_channel_name }}</b> | ID: <code>{{ alert_channel_id }}</code>)<br>'
            '{{ form.alert_role.label }} {{ form.alert_role() }} '
            '(<b>{{ alert_role_name }}</b> | ID: <code>{{ alert_role_id }}</code>)<br>'
        ...
        return {
            "status": 0,
            "web_content": {
                "source": custom_form_html,
                "form": form,
                "alert_channel_name": alert_channel_name,
                "alert_channel_id": alert_channel_id or "Not set",
                "alert_role_name": alert_role_name,
                "alert_role_id": alert_role_id or "Not set",
            },
        } 