...
class Skysearch(commands.Cog, DashboardIntegration):
                        ...
                        for guild in guilds:
                            ...
                            if alert_channel_id:
                                ...
                                if alert_channel:
                                    ...
                                    # Create buttons for emergency alerts using helper methods
                                    icao = aircraft_data.get('hex', '')
                                    view = self.helpers.create_base_aircraft_view(icao)
                                    self.helpers.add_social_media_buttons(view, aircraft_data, squawk_code=squawk_code)
    ...
    async def _send_custom_alert(self, alert_channel, guild_config, aircraft_info, alert_data, alert_id):
        ...
        try:
            ...
            icao = aircraft_data.get('hex', '')
            view = self.helpers.create_base_aircraft_view(icao)
            self.helpers.add_social_media_buttons(
                view, aircraft_data, 
                alert_type=alert_data['type'], 
    ...
    async def simulate_emergency_alert(self, ctx, hex_code: str, squawk_code: str = "7700", callsign: str = "TEST123"):
        ...
        # Create buttons using helper methods (same as real emergency alerts)
        icao = aircraft_data.get('hex', '')
        view = self.helpers.create_base_aircraft_view(icao)
        self.helpers.add_social_media_buttons(view, aircraft_data, squawk_code=squawk_code)
