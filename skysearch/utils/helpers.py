...
class JSONInputModal(discord.ui.Modal):
    ...
    def convert_speed_knots_to_mph(self, ground_speed_knots):
        """
        Convert ground speed from knots to miles per hour.
        
        Args:
            ground_speed_knots: Speed in knots (can be int, float, str, or 'N/A')
            
        Returns:
            int or str: Speed in mph, or 'unknown' if conversion fails
        """
        if ground_speed_knots == 'N/A' or ground_speed_knots is None:
            return 'unknown'
        try:
            return round(float(ground_speed_knots) * 1.15078)
        except (ValueError, TypeError):
            return 'unknown'
    
    def format_coordinate(self, coord, coord_type='lat'):
        """
        Format a coordinate (latitude or longitude) for display.
        
        Args:
            coord: Coordinate value (can be float, int, str, or 'N/A')
            coord_type: 'lat' for latitude, 'lon' for longitude
            
        Returns:
            str: Formatted coordinate (e.g., "40.71N" or "-74.01W") or original value if invalid
        """
        if coord == 'N/A' or coord is None:
            return 'N/A'
        try:
            coord_float = round(float(coord), 2)
            if coord_type == 'lat':
                coord_dir = "N" if coord_float >= 0 else "S"
            else:  # lon
                coord_dir = "E" if coord_float >= 0 else "W"
            return f"{abs(coord_float)}{coord_dir}"
        except (ValueError, TypeError):
            return coord
    
    def create_base_aircraft_view(self, icao_hex):
        """
        Create a base Discord view with the airplanes.live link button.
        
        Args:
            icao_hex: ICAO hex code (string)
            
        Returns:
            discord.ui.View: View with airplanes.live button
        """
        view = discord.ui.View()
        if icao_hex:
            icao_hex = icao_hex.upper()
        link = f"https://globe.airplanes.live/?icao={icao_hex}"
        view.add_item(discord.ui.Button(
    ...
    def add_social_media_buttons(self, view, aircraft_data, squawk_code=None, alert_type=None, alert_value=None):
        """
        Add social media sharing buttons (X/Twitter and WhatsApp) to a view.
        
        Args:
            view: discord.ui.View to add buttons to
            aircraft_data: Dictionary containing aircraft information
            squawk_code: Optional squawk code (for emergency detection)
            alert_type: Optional alert type (for custom alerts)
            alert_value: Optional alert value (for custom alerts)
            
        Returns:
            discord.ui.View: View with social media buttons added
        """
        from urllib.parse import quote_plus
        
        # Get formatted data
        ground_speed_knots = aircraft_data.get('gs', aircraft_data.get('ground_speed', 'N/A'))
        ground_speed_mph = self.convert_speed_knots_to_mph(ground_speed_knots)
        
        lat = self.format_coordinate(aircraft_data.get('lat', 'N/A'), 'lat')
        lon = self.format_coordinate(aircraft_data.get('lon', 'N/A'), 'lon')
        
        flight = aircraft_data.get('flight', '')
        icao = aircraft_data.get('hex', '').upper()
        
        # Determine tweet text based on context
        emergency_squawk_codes = ['7500', '7600', '7700']
        discord_invite = "https://discord.gg/X8huyaeXrA"  # Standard Discord invite URL
        if squawk_code and squawk_code in emergency_squawk_codes:
            tweet_text = (
                f"Spotted an aircraft declaring an emergency! #Squawk #{squawk_code}, "
                f"flight {flight} at position {lat}, {lon} with speed {ground_speed_mph} mph. "
                f"#SkySearch #Emergency\n\n"
                f"Join via Discord to search and discuss planes with your friends for free - "
                f"{discord_invite}"
            )
        elif alert_type and alert_value:
            tweet_text = (
                f"alert triggered! {alert_type.upper()} '{alert_value}' spotted - "
                f"Flight {flight} at position {lat}, {lon} with speed {ground_speed_mph} mph. "
                f"#SkySearch #discordAlert\n\n"
                f"Join via Discord to search and discuss planes with your friends for free - "
                f"{discord_invite}"
            )
        else:
            tweet_text = (
                f"Tracking flight {flight} at position {lat}, {lon} with speed {ground_speed_mph} mph "
                f"using #SkySearch\n\n"
                f"Join via Discord to search and discuss planes with your friends for free - "
                f"{discord_invite}"
            )
        ...
        if alert_type and alert_value:
            whatsapp_text = (
                f"Custom alert! {alert_type.upper()} '{alert_value}' spotted - "
                f"Flight {flight} at position {lat}, {lon} with speed {ground_speed_mph} mph. "
                f"Track live @ https://globe.airplanes.live/?icao={icao} #SkySearch"
            )
        else:
            whatsapp_text = (
                f"Check out this aircraft! Flight {flight} at position {lat}, {lon} with speed {ground_speed_mph} mph. "
                f"Track live @ https://globe.airplanes.live/?icao={icao} #SkySearch"
            )
