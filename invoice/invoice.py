...
class Invoice(commands.Cog):
    ...
    async def generate_invoice(self, ctx, client_name: str, amount: float, due_date: str, *items):
        try:
            ...
            pdf_path = await self.create_invoice_pdf(invoice_number, invoice_date, client_name, amount, due_date, items, pdf_filename)
    ...
    async def create_invoice_pdf(self, invoice_number, invoice_date, client_name, amount, due_date, items, pdf_filename):
        # Create PDF
